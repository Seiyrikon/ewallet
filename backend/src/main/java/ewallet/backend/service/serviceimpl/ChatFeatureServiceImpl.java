    ...
    public ResponseEntity<Map<String, Object>> chats(Long user_id) {
        ...
        try 
        {
            if (authentication != null && authentication.isAuthenticated()) 
            {
                ...
                if(chats.size() != 0)
                {
                    Collections.sort(chats, Comparator.comparing(ChatSessionDto::created_at));
    ...
    public ResponseEntity<Map<String, Object>> getChatSession(Long user_id) {
        ...
        try 
        {
            if (authentication != null && authentication.isAuthenticated()) 
            {
                ...
                List<ChatSessionDto> chatSession = tbl_chatDao.getChatSession(userId, user_id)
                .stream()
                .map(chatSessionDtoMapper).collect(Collectors.toList());
    
                if(chatSession.size() != 0)
                {
                    Collections.sort(chatSession, Comparator.comparing(ChatSessionDto::created_at));
                    response.put("message", chatSession);
                }
                else
                {
                    tbl_user_mst user = tbl_user_mstDao.getUserById(user_id);
